...
int main(int argc, char *argv[])
{
    ...
    puts("=================== Input query path or press Ctrl+C to exit ====================");
    ...
    while (getline(cin, command_line))
    {
      ...
      while (ss >> arg_ele)    argv_vec.push_back(arg_ele);

      if (argc >= 1)
      {
          string query_path = argv_vec[0];

          string query = Util::getQueryFromFile(query_path.data());

          if (query.empty())      
          {
            cout << "Invalid query path!" << endl;
            continue;
          }
          printf("query is:\n%s\n\n", query.c_str());
          ResultSet _rs;
          FILE *ofp = stdout;
          if (argc >= 4)          ofp = fopen(argv_vec[1].data(), "w");
          string msg;
          // int ret = _db.query(query, _rs, ofp);
          int ret = _db.query(query, _rs, ofp, true, false, true, query_path);
          if (argc >= 4)
          {
              fclose(ofp);
              ofp = NULL;
